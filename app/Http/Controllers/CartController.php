...
class CartController extends Controller
{
    ...
    protected function cartQuery($withProduct = false)
    {
        return Helper::activeCartQuery('', $withProduct);
    }

    protected function assignCartOwner(Cart $cart)
    {
        if (auth()->check()) {
        ...
        $cart->user_id = null;
        $cart->session_id = Helper::guestCartToken();
    }

    protected function extractQuantity(Request $request)
    {
    ...
    public function addToCart(Request $request){
        ...
        $already_cart = $this->cartQuery()->where('product_id', $product->id)->first();
        // return $already_cart;
        if($already_cart) {
            // dd($already_cart);
    ...
    public function singleAddToCart(Request $request){
        ...
        if($already_cart) {
            ...
            $already_cart->amount = ($already_cart->price * $quantity) + $already_cart->amount;
    ...
    public function cartDelete(Request $request){
        $cart = $this->cartQuery()->find($request->id);
        if ($cart) {
            $cart->delete();
