  ...
  <section class="order_details pt-3">
    ...
    <table class="table table-bordered table-stripe">
      ...
      @foreach($order->cart_info as $cart)
        <tr>
          <td><span>{{optional($cart->product)->title ?? 'Product removed'}}</span></td>
          <td>x{{$cart->quantity}}</td>
          <td><span>{{ Helper::currency($cart->price) }}</span></td>
        </tr>
      @endforeach
      </tbody>
      <tfoot>
        <tr>
          ...
          <th scope="col"> <span>{{ Helper::currency($order->sub_total) }}</span></th>
        </tr>
      {{-- @if(!empty($order->coupon))
        <tr>
          <th scope="col" class="empty"></th>
          <th scope="col" class="text-right">Discount:</th>
          <th scope="col"><span>-{{ Helper::currency($order->coupon->discount(Helper::orderPrice($order->id, $order->user->id))) }}</span></th>
        </tr>
      @endif --}}
        <tr>
          <th scope="col" class="empty"></th>
          <th scope="col" class="text-right ">Shipping:</th>
          <th><span>{{ Helper::currency(optional($order->shipping)->price ?? 0) }}</span></th>
        </tr>
        <tr>
          <th scope="col" class="empty"></th>
          ...
          <th>
            <span>
                {{ Helper::currency($order->total_amount) }}
            </span>
          </th>
        </tr>
      </tfoot>
