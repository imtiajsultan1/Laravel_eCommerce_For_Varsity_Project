                                ...
                                <div class="row">
                                    ...
                                    <div class="col-lg-6 col-md-6 col-12">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="phone" placeholder="" required value="{{old('phone', $lastOrder ? $lastOrder->phone : '')}}">
                                            @error('phone')
                                                <span class='text-danger'>{{$message}}</span>
                                            @enderror
                                        </div>
                        ...
                        <div class="col-lg-4 col-12">
                            <div class="order-details">
                                ...
                                <div class="single-widget">
                                    ...
                                    <div class="content">
                                        <ul>
										    <li class="order_subtotal" data-price="{{Helper::totalCartPrice()}}">Cart Subtotal<span>{{ Helper::currency(Helper::totalCartPrice()) }}</span></li>
                                            <li class="shipping">
                                                Shipping Cost
                                                @if(count(Helper::shipping())>0 && Helper::cartCount()>0)
                                                    <select name="shipping" class="nice-select" required>
                                                        ...
                                                        <option value="{{$shipping->id}}" class="shippingOption" data-price="{{$shipping->price}}">{{$shipping->type}}: {{ Helper::currency($shipping->price) }}</option>
                                                        @endforeach
                                                    </select>
                                                @else 
                                                    <span>Free</span>
                                            ...
                                            @if(session('coupon'))
                                            <li class="coupon_price" data-price="{{session('coupon')['value']}}">You Save<span>{{ Helper::currency(session('coupon')['value']) }}</span></li>
                                            @endif
                                            @php
                                            ...
                                            @endphp
                                            @if(session('coupon'))
                                                <li class="last"  id="order_total_price">Total<span>{{ Helper::currency($total_amount) }}</span></li>
                                            @else
                                                <li class="last"  id="order_total_price">Total<span>{{ Helper::currency($total_amount) }}</span></li>
                                            @endif
                                        </ul>
	...
	<script>
		$(document).ready(function(){
			$('.shipping select[name=shipping]').change(function(){
				...
				// alert(coupon);
				$('#order_total_price span').text('{{ Helper::base_currency() }} ' + (subtotal + cost-coupon).toFixed(2));
			});
